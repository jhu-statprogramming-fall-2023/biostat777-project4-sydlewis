...
output$myPlot <- renderPlot({
  ...
  filtered_data <- ushouse_v1 %>%
    filter(if(input$stateInput != "All States") state == input$stateInput else TRUE,
           ...
           stage == "GEN") %>%
    group_by(party_dro, year, year_type) %>%
    summarise(mean_votes = mean(candidatevotes), .groups = 'drop') %>%
    ungroup()

  # Create the plot
  ggplot(filtered_data, aes(x = year, y = mean_votes, color = party_dro, group = interaction(party_dro, year_type), shape = year_type)) +
    geom_point(size = 2.5) +
    geom_line() +
    scale_color_manual(values = c("Republican" = "red", "Democrat" = "blue", "Other" = "gray")) +
    labs(
...
output$myInteractivePlot <- renderPlot({
  ...
  # Debug: print the first few lines of the filtered data to the console
  print(head(filtered_data))

  # Create the plot
  ggplot(filtered_data, aes(x = ifelse(party_dro == "Democrat", -percentage, percentage), y = factor(year, levels = rev(unique(year))), fill = party_dro)) +
    geom_col(position = "stack") +
    labs(
      title = "Percentage of Wins per Party Across Election Years in House Races",
      subtitle = "A 'win' is defined as receiving the most votes among other candidates in that general election",
    ...
    scale_fill_manual(values = c("Democrat" = "blue", "Republican" = "red")) +
    scale_x_continuous(limits = c(-110, 100), labels = function(x) abs(x)) +
    theme(
      legend.position = "top", 
      legend.justification = "center"  
    )
