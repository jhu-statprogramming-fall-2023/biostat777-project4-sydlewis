...
# UI
ui <- fluidPage(
    titlePanel("Interactive State-wise Vote Visualization"),
...
server <- function(input, output) {
    output$votePlot <- renderPlot({
        ...
        ggplot(filtered_data, aes(x = year, y = total_votes, fill = year_type)) +
            geom_col() +
            labs(
                title = paste("Total Votes per Party Across Election Years in House Races in", input$state),
...
server <- function(input, output) {
    ...
    output$mapOutput <- renderPlot({
        ...
        labs(title = "Election Wins Across State by Year",
             subtitle = "Use the drop down menu to select a year",
